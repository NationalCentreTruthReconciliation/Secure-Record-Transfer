...
class TestMetadata(TestCase):
    ...
    def test_flatten_metadata_full_section_1_related_caais_1_0(self):
        ...
        identifier_2 = Identifier(
            metadata=metadata,
            identifier_type='ID',
            identifier_note='Identifier to be received'
        )
        archival_unit_1 = ArchivalUnit(
            metadata=metadata,
            archival_unit='Unit 1'
        )
        archival_unit_2 = ArchivalUnit(
            metadata=metadata,
            archival_unit='Unit 2'
        )
        ...
        objects = (
            acquisition_method, status, metadata, identifier_1, identifier_2, archival_unit_1,
            archival_unit_2, disposition_authority
        )
        for obj in objects:
            obj.save()
    ...
    def test_flatten_metadata_full_section_3_related_caais_1_0(self):
        ...
        objects = (
            metadata, extent_type, content_type, carrier_type, extent_1,
            extent_2, language_1, language_2
        )
        for obj in objects:
            obj.save()
    ...
    def test_flatten_metadata_full_section_4_related_caais_1_0(self):
        ...
        objects = (
            metadata, location_1, location_2, rights_type, rights, req_type,
            preservation_requirement, appraisal_type, appraisal_1, appraisal_2,
            documentation_1,
        )
        ...
        self.assertEqual(flat['preservationRequirementsValues'], 'Handle in accordance with other digital material')
        ...
        self.assertEqual(flat['appraisalValues'], '9/10 PDF files are of archival value|1 XLSX file is archivally relevant')
        self.assertEqual(flat['appraisalNotes'], 'PDF file that is not relevant is named letter.pdf|NULL')
        ...
        self.assertEqual(flat['associatedDocumentationNotes'], 'Subject line of email sent to repository prior to submission')

        for obj in objects:
            obj.delete()


    @patch.object(timezone, 'now', return_value=datetime(2023, 12, 5, 12, 0, 0, tzinfo=timezone.get_current_timezone()))
    def test_flatten_metadata_full_section_5_related_caais_1_0(self, timezone_now__patch):
        ...
        objects = (
            metadata, event_type_1, event_type_2, event_1, event_2
        )
        for obj in objects:
            obj.save()
    ...
    @patch.object(timezone, 'now', return_value=datetime(2023, 5, 23, 12, 0, 0, tzinfo=timezone.get_current_timezone()))
    def test_flatten_metadata_full_section_7_caais_1_0(self, timezone_now__patch):
        ...
        self.assertEqual(flat['creationOrRevisionAgents'], 'Transfer application|Transfer application')
        self.assertEqual(flat['creationOrRevisionNotes'], 'NULL|Revised to meet repository standards')
        self.assertEqual(flat['languageOfAccessionRecord'], 'en')
