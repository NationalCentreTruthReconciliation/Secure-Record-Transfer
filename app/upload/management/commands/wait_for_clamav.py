...
from upload.clam import get_clamd_socket
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser: CommandParser) -> None:
        ...
        parser.add_argument(
            ...
            default=5,
            metavar="TIMES",
            help=("max number of times to retry pinging ClamAV before failing (default: 5)"),
        )
        parser.add_argument(
            "--retry-interval",
            type=int,
    ...
    def handle(self, *args, **options) -> None:
        """Run after arguments are parsed."""
        ...
        while not ping_success and attempts < max_retries:
            try:
                ...
                self.stdout.write(
                    self.style.WARNING(f"Connection not available, waiting {interval} seconds ...")
                )
                time.sleep(interval)
